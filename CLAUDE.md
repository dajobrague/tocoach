...
Airtable was fully removed in October 2025 (see `docs/architecture/auth-adr-v2.md`). Trainers are Supabase Auth users (`auth.users`, login via GoTrue) with a `trainers` row keyed by `tenant_host`. Clients are **not** in Supabase Auth: they live only in `clients` (`tenant` uuid → `tenants.id`) with their own credentials and the app's own client JWT (`lib/auth/client-session.ts`). There are no `trainer_profiles` / `client_profiles` / `trainer_clients` tables. **Tenant isolation is the `tenant_host` / `tenant_slug` filter in each service, not RLS.** Every server query goes through the service-role factory `lib/clients/supabase-admin.ts` (guarded by `server-only`; `supabase-server.ts` / `supabase-api.ts` delegate to it), which bypasses RLS — a query without the tenant filter reads every tenant. When adding a new table, add a migration under `supabase/migrations/` (numbered, sequential) and filter by tenant in the service; do not grant anything to `anon` / `authenticated`.
