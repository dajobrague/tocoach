...
export async function POST(
  ...
  try {
    ...
    // Create impersonation URL that goes directly to trainer's subdomain/slug
    // This way the session cookie is set on their path, not the admin path
    const isDev = process.env.NODE_ENV === "development";
    const impersonationUrl = isDev
      ? `http://localhost:3000/${tenant.slug}/auth/impersonate?token=${impersonationToken}`
      : `https://${tenant.host}/auth/impersonate?token=${impersonationToken}`;
