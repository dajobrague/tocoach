...
// Use anon key for all operations
...
export async function POST(request: NextRequest) {
    try {
        ...
        try {
            ...
            return NextResponse.json(
                { error: error instanceof Error ? error.message : 'Invalid invitation code' },
                { status: 400 }
            );
        }
        ...
        if (authError || !authUser.user) {
            ...
            return NextResponse.json(
                { error: 'Error al crear la cuenta. Es posible que el correo electrónico ya esté en uso.' },
                { status: 400 }
            );
        }
        ...
        if (trainerError) {
            ...
            console.error('[Registration] Trainer creation failed, user may exist without trainer record');
        ...
        console.log(`[Registration] Successfully created trainer: ${email} with temporary tenant: ${tempTenantHost}`);
