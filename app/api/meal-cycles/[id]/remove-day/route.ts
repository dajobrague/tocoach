...
import { parseRemoveDay } from "@/lib/nutrition/cycles/cycle-request";
import {
  MealCycleService,
...
// POST /api/meal-cycles/[id]/remove-day — drop a day and renumber the later
// days down (no gaps). The removed day's meals are deleted.
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    const parsed = parseRemoveDay(body);
    ...
    const result = await service.removeDay(
    ...
    console.error(`${LOG_PREFIX} remove day error:`, {
