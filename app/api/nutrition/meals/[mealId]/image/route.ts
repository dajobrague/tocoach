...
/**
 * Extract storage object path from a Supabase public URL for meal-images.
 * Returns null if URL is not for this project's meal-images bucket.
 */
...
async function verifyMealTrainerOwnership(
  ...
  | {
      ok: true;
      meal: { id: string; nutrition_day_id: string; image_url: string | null };
    }
  | { ok: false; status: number; error: string }
> {
  const { data: meal, error: mealError } = await supabase
    .from("nutrition_meals")
    .select("id, nutrition_day_id, image_url")
    .eq("id", mealId)
    .single();

  if (mealError || !meal) {
    console.error("[Meal Image] Meal not found:", mealError);
  ...
  if (dayError || !day) {
    console.error("[Meal Image] Day not found:", dayError);
  ...
  if (planError || !plan) {
    console.error("[Meal Image] Plan not found or unauthorized:", planError);
...
/**
 * POST /api/nutrition/meals/[mealId]/image
...
export async function POST(
  ...
  try {
    ...
    const ownership = await verifyMealTrainerOwnership(
      supabase,
      mealId,
      session.trainer_id
    );
    ...
    const storagePath = `${session.trainer_id}/${mealId}/${timestamp}-${randomId}.${ext}`;
    ...
    if (uploadError) {
      console.error("[Meal Image] Upload error:", uploadError);
    ...
    if (updateError) {
      console.error("[Meal Image] DB update error:", updateError);
    ...
    const previousUrl = ownership.meal.image_url;
    ...
    if (previousUrl) {
      ...
      if (oldPath) {
        ...
        if (removeError) {
          console.error(
            "[Meal Image] Failed to remove previous object:",
            removeError
          );
        }
      }
    ...
    console.error("[Meal Image] Unexpected error:", error);
...
/**
 * DELETE /api/nutrition/meals/[mealId]/image
 */
export async function DELETE(
  ...
  try {
    ...
    const imageUrl = ownership.meal.image_url;
    ...
    if (path) {
      ...
      if (removeError) {
        console.error("[Meal Image] Storage remove error:", removeError);
      }
    }
    ...
    if (updateError) {
      console.error("[Meal Image] DB clear image_url error:", updateError);
