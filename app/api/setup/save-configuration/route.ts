...
export async function POST(request: NextRequest) {
    try {
        ...
        if (missingKeys.length > 0) {
            console.error('[Save Configuration] Missing required theme keys:', missingKeys);
            return NextResponse.json(
                { success: false, error: `Configuración incompleta. Faltan: ${missingKeys.join(', ')}` },
                { status: 400 }
            );
        }
        ...
        if (trainerError) {
            ...
            return NextResponse.json(
                { success: false, error: 'Error al actualizar el perfil del entrenador' },
                { status: 500 }
            );
        }
        ...
        if (existingTenant) {
            ...
            console.log('[Save Configuration] Updating existing tenant:', existingTenant.host);
            ...
            // Create new tenant
            console.log('[Save Configuration] Creating new tenant for domain:', normalizedDomain);
