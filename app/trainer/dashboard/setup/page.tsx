...
import { SetupWizardProvider, useSetupWizard } from '@/lib/setup-wizard/context';
            ...
            <div className="w-1/2 flex flex-col bg-white border-r border-gray-200 overflow-hidden">
                ...
                <div className="px-6 py-4 border-b border-gray-200 flex-shrink-0">
                    <div className="flex items-center justify-between mb-4">
                        <div>
                            <h1 className="text-2xl font-heading font-bold text-black">Configuración de Plataforma</h1>
                            <p className="text-gray-600 font-body">Personaliza tu experiencia de coaching</p>
                        </div>
                        <Button
                            isIconOnly
                            variant="light"
                    ...
                    <div className="space-y-3">
                        <div className="flex justify-between text-sm">
                            <span className="text-gray-600">Paso {state.currentStep} de {STEPS.length}</span>
                            <span className="text-gray-600">{Math.round((state.currentStep / STEPS.length) * 100)}%</span>
                        </div>
                        <Progress
                    ...
                    <div className="grid grid-cols-3 gap-2 mt-4">
                        {STEPS.map((step, index) => (
                            <div
                                key={step.key}
                                className={`flex items-center gap-2 px-3 py-2 rounded-lg ${state.currentStep === step.key
                            ...
                            >
                                <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${state.currentStep === step.key
                                    ...
                                    {state.currentStep > step.key ? (
                                        <Icon icon="solar:check-linear" className="text-xs" />
                                    ) : (
                                        step.key
                                    )}
                                </div>
                                <div className="text-left min-w-0 flex-1">
                                    <p className={`text-sm font-medium truncate ${state.currentStep >= step.key ? 'text-black' : 'text-gray-500'
                                        }`}>
                                        {step.title}
                                    </p>
                ...
                {/* Step Content */}
                <div className="flex-1 overflow-auto p-6" style={{ overscrollBehavior: 'contain' }}>
                    {renderCurrentStep()}
                </div>
            </div>
...
export default function SetupWizardPage() {
            ...
            .then(async (sessionData) => {
                if (sessionData.session) {
                    ...
                    if (sessionData.session.onboarding_completed) {
                        console.log('[Setup Wizard] Onboarding already completed, redirecting to dashboard');
                    ...
                    try {
                        ...
                        console.warn('[Setup Wizard] Failed to load config, using defaults:', error);
    ...
    const currentDomain = config?.domain?.current || trainerSession.tenant_host || 'temp-domain.localhost';
    ...
    const initialData = {
        ...
        logo: {
            ...
            text: config?.theme?.meta?.logoText || config?.trainer?.fullName || trainerSession.full_name || 'MI MARCA',
            position: (config?.theme?.logo?.position as 'left' | 'center' | 'right') || 'left',
            size: (config?.theme?.logo?.size as 'small' | 'medium' | 'large') || 'medium',
        },
