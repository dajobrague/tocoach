...
export default function TrainerForgotPasswordPage() {
  ...
  const postForgotPassword = async (emailToSend: string) => {
    const res = await fetch("/api/auth/trainer-forgot-password", {
      method: "POST",
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/auth/trainer-verify-otp", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          email,
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/auth/trainer-reset-password", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          email,
          ...
          >
            ...
            <div className="w-1/4 shrink-0 px-5 py-7 sm:px-8 sm:py-9">
              ...
              <div className="mt-6 text-center">
                <Link
                  className="text-black font-semibold font-body"
                  href="/trainer/login"
                  size="sm"
                >
            ...
            <div className="w-1/4 shrink-0 px-5 py-7 sm:px-8 sm:py-9">
              ...
              <div className="mt-6 flex flex-col items-center gap-3 border-t border-default-100 pt-6">
                {resendCooldown > 0 ? (
                  ...
                  <button
                    className="font-body text-sm font-semibold text-foreground underline-offset-4 transition-opacity hover:opacity-80 disabled:opacity-40"
                    disabled={busy}
                    type="button"
            ...
            <div className="w-1/4 shrink-0 px-5 py-7 sm:px-8 sm:py-9">
              {error && step === 3 && (
                <div className="mb-4 rounded-lg border border-danger-200 bg-danger-50 p-3">
                  ...
                  {resetTokenExpired && (
                    <button
                      className="mt-3 font-body text-sm font-semibold text-foreground underline-offset-2 hover:underline"
                      type="button"
                      onClick={goBackToEmail}
                    >
              ...
              >
                ...
                <div className="rounded-lg border border-default-200 bg-default-50/80 p-4">
            ...
            <div className="flex w-1/4 shrink-0 flex-col items-center justify-center px-5 py-12 sm:px-8 sm:py-16">
              ...
              <Link
                className="mt-8 text-black font-semibold font-body"
                href="/trainer/login"
                size="sm"
              >
