...
import { Avatar, Button, Card, CardBody, Tabs, Tab } from '@heroui/react';
...
export function DashboardContent({ firstName, logoUrl, trainerName, clientProfilePicture }: DashboardContentProps) {
    // State
    ...
    const formatDateForPeriod = (index: number, totalPoints: number, period: string): string => {
        const now = new Date();
        ...
        <>
            <div className="min-h-screen bg-background pb-20">
                <div className="max-w-lg mx-auto">
                    ...
                    <div className="mb-4">
                        <h2 className="text-lg font-semibold font-heading mb-3 px-4 text-foreground">Registro Diario</h2>
                        <div
                            onClick={() => setShowCheckInModal(true)}
                            className="bg-primary cursor-pointer hover:opacity-90 transition-all active:scale-[0.98] py-8 px-6"
                        >
                            <div className="max-w-lg mx-auto flex items-center justify-between">
                                <span className="text-white text-xl font-medium">
                                    Registra tu día de hoy
                                </span>
                                <Icon icon="solar:alt-arrow-right-bold" className="text-white text-3xl" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="px-4 space-y-4">
                        <h2 className="text-lg font-semibold font-heading text-foreground">Progreso</h2>
                            ...
                            <CardBody className="p-4">
                                <div className="flex items-center justify-between mb-2">
                                    ...
                                    <div className="bg-warning/10 p-1.5 rounded-full">
                                        <Icon icon="solar:body-bold" className="text-warning text-base" />
                                    </div>
                                </div>
                                <p className="text-5xl font-bold mb-1 text-foreground">{weightHistory[weightHistory.length - 1]?.weight}</p>
                                ...
                                <div className="flex items-end justify-between gap-2 h-24">
                                    {weightHistory.map((day, index) => {
                                        ...
                                        return (
                                            <div key={`weight-${index}`} className="flex-1 flex flex-col items-center gap-2">
                            ...
                            <CardBody className="p-4">
                                <div className="flex items-center justify-between mb-2">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        >
                                            <Icon icon="solar:minus-circle-bold" className="text-base" />
                                        </Button>
                                        <Button
                                            isIconOnly
                                            size="sm"
                                        ...
                                        <div className="bg-primary p-1.5 rounded-full">
                                            <Icon icon="solar:bottle-bold" className="text-white text-base" />
                                    </div>
                                    </div>
                                </div>
                                ...
                                <p className="text-sm text-foreground/70 mb-4">L de {waterGoal} L</p>
                                ...
                                <div className="flex justify-between mt-2">
                                    ...
                                    <span className="text-xs text-foreground/70 font-semibold">{waterPercentage.toFixed(0)}%</span>
                            ...
                            <CardBody className="p-4">
                                <div className="flex items-center justify-between mb-2">
                                    ...
                                    <div className="bg-secondary/10 p-1.5 rounded-full">
                                        <Icon icon="solar:moon-sleep-bold" className="text-secondary text-base" />
                                    </div>
                                </div>
                                <p className="text-5xl font-bold mb-1 text-foreground">{sleepHistory[sleepHistory.length - 1]?.hours}</p>
                                ...
                                <div className="flex items-end justify-between gap-2 h-24">
                                    {sleepHistory.map((day, index) => {
                                        ...
                                        return (
                                            <div key={`sleep-${index}`} className="flex-1 flex flex-col items-center gap-2">
                                                <div className="w-full relative" style={{ height: '80px' }}>
                                                    <div
                            ...
                            <CardBody className="p-4">
                                <div className="flex items-center justify-between mb-2">
                                    ...
                                    <div className="bg-danger/10 p-1.5 rounded-full">
                                        <Icon icon="solar:fire-bold" className="text-danger text-base" />
                                    </div>
                                </div>
                                <p className="text-5xl font-bold mb-1 text-foreground">{calorieHistory[calorieHistory.length - 1]?.calories}</p>
                                <p className="text-sm text-foreground/70 mb-4">Hoy</p>
                                <div className="flex items-end justify-between gap-2 h-24">
                                    {calorieHistory.map((day, index) => {
                                        const maxCals = Math.max(...calorieHistory.map(d => d.calories));
                                        const height = (day.calories / maxCals) * 100;
                                        const isToday = index === calorieHistory.length - 1;
                                        return (
                                            <div key={`calorie-${index}`} className="flex-1 flex flex-col items-center gap-2">
                                                <div className="w-full relative" style={{ height: '80px' }}>
                                                    <div
                        ...
                        <CardBody className="p-4">
                                <div className="flex items-center justify-between mb-2">
                                    ...
                                    <div className="bg-success/10 p-1.5 rounded-full">
                                        <Icon icon="solar:walking-bold" className="text-success text-base" />
                                    </div>
                                </div>
                                <p className="text-5xl font-bold mb-1 text-foreground">{todaySteps}</p>
                                <p className="text-sm text-foreground/70 mb-4">Hoy</p>
                                <div className="flex items-end justify-between gap-2 h-24">
                                    {stepsHistory.map((day, index) => {
                                        const maxSteps = Math.max(...stepsHistory.map(d => d.steps));
                                        const height = (day.steps / maxSteps) * 100;
                                        const isToday = index === stepsHistory.length - 1;
                                        return (
                                            <div key={`steps-${index}`} className="flex-1 flex flex-col items-center gap-2">
                                                <div className="w-full relative" style={{ height: '80px' }}>
                                                    <div
