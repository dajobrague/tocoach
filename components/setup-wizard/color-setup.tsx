...
export default function ColorSetup() {
    ...
    const handlePaletteSelect = (palette: typeof COLOR_PALETTES[0]) => {
        ...
        actions.setButtonColor('secondary', 'hover', palette.buttons.secondary.hover);
        ...
        <div className="space-y-3">
            ...
            <div className="flex items-center gap-4">
                <Button
                    ...
                    onPress={() => setActiveColorPicker(activeColorPicker === pickerId ? null : pickerId)}
                >
                    <div className="w-full h-full rounded-lg" style={{ backgroundColor: color }}></div>
                </Button>
                <Input
                    value={color}
                    onValueChange={(value) => handleHexInput(value, () => onChange(value))}
                    placeholder="#3b82f6"
        ...
        <div className="flex flex-col gap-6 w-full">
            ...
            <div>
                ...
                <p className="text-gray-600 font-body">
                    Personaliza todos los aspectos visuales de tu plataforma para crear una experiencia de marca cohesiva
                </p>
            </div>

            {/* Predefined Palettes */}
            <div>
                <h3 className="text-lg font-heading font-semibold text-black mb-4">Paletas predefinidas</h3>
            ...
            <div className="space-y-8">
                <h3 className="text-lg font-heading font-semibold text-black">Personalización avanzada</h3>

                {/* Brand Colors Section */}
                <div>
                    <h4 className="text-md font-semibold text-black mb-4 flex items-center gap-2">
                        <Icon icon="solar:palette-linear" className="text-blue-600" />
                        Colores de Marca
                    </h4>
                ...
                <div>
                    <h4 className="text-md font-semibold text-black mb-4 flex items-center gap-2">
                        <Icon icon="solar:layers-linear" className="text-blue-600" />
                        Colores de Fondo
                    </h4>
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                        <ColorPickerButton
                        ...
                        <ColorPickerButton
                            ...
                            onChange={(color) => actions.setBackgroundColor('secondary', color)}
                        />
                        <ColorPickerButton
                ...
                <div>
                    <h4 className="text-md font-semibold text-black mb-4 flex items-center gap-2">
                        <Icon icon="solar:cursor-linear" className="text-blue-600" />
                        Colores de Botones
                    </h4>

                    {/* Primary Button */}
                    <div className="mb-6">
                        <h5 className="text-sm font-medium text-gray-700 mb-3">Botón Primario</h5>
                        <div className="grid md:grid-cols-3 gap-4">
                            <ColorPickerButton
                                color={state.colors.buttons.primary.bg}
                                label="Fondo"
                                pickerId="btn-primary-bg"
                                onChange={(color) => actions.setButtonColor('primary', 'bg', color)}
                            />
                            <ColorPickerButton
                                color={state.colors.buttons.primary.text}
                                label="Texto"
                                pickerId="btn-primary-text"
                                onChange={(color) => actions.setButtonColor('primary', 'text', color)}
                            />
                            <ColorPickerButton
                                color={state.colors.buttons.primary.hover}
                                label="Hover"
                                pickerId="btn-primary-hover"
                                onChange={(color) => actions.setButtonColor('primary', 'hover', color)}
                            />
                        </div>
                    </div>
                    ...
                    {/* Secondary Button */}
                    <div>
                        <h5 className="text-sm font-medium text-gray-700 mb-3">Botón Secundario</h5>
                        <div className="grid md:grid-cols-3 gap-4">
                            <ColorPickerButton
                                color={state.colors.buttons.secondary.bg}
                                label="Fondo"
                                pickerId="btn-secondary-bg"
                                onChange={(color) => actions.setButtonColor('secondary', 'bg', color)}
                            />
                            <ColorPickerButton
                                color={state.colors.buttons.secondary.text}
                                label="Texto"
                                pickerId="btn-secondary-text"
                                onChange={(color) => actions.setButtonColor('secondary', 'text', color)}
                            />
                            <ColorPickerButton
                                color={state.colors.buttons.secondary.hover}
                                label="Hover"
                                pickerId="btn-secondary-hover"
                                onChange={(color) => actions.setButtonColor('secondary', 'hover', color)}
                            />
                        </div>
                    </div>
                </div>
            ...
            <Card className="bg-gray-50 border border-gray-200">
                <CardBody className="p-6">
                    <h4 className="text-lg font-semibold text-black mb-4">Resumen de colores</h4>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        {/* Brand Colors */}
                        <div className="space-y-2">
                            <p className="text-xs font-medium text-gray-600 uppercase">Marca</p>
                            <div className="flex items-center gap-2">
                                <div className="w-4 h-4 rounded-full border border-gray-300" style={{ backgroundColor: state.colors.primary }}></div>
                                <span className="text-xs text-gray-600">Primario</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <div className="w-4 h-4 rounded-full border border-gray-300" style={{ backgroundColor: state.colors.secondary }}></div>
                        ...
                        {/* Text Colors */}
                        <div className="space-y-2">
                            <p className="text-xs font-medium text-gray-600 uppercase">Texto</p>
                            <div className="flex items-center gap-2">
                                <div className="w-4 h-4 rounded-full border border-gray-300" style={{ backgroundColor: state.colors.text.h1 }}></div>
                                <span className="text-xs text-gray-600">H1</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <div className="w-4 h-4 rounded-full border border-gray-300" style={{ backgroundColor: state.colors.text.body }}></div>
                        ...
                        {/* Background Colors */}
                        <div className="space-y-2">
                            <p className="text-xs font-medium text-gray-600 uppercase">Fondos</p>
                            <div className="flex items-center gap-2">
                                <div className="w-4 h-4 rounded-full border border-gray-300" style={{ backgroundColor: state.colors.background.primary }}></div>
                                <span className="text-xs text-gray-600">Principal</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <div className="w-4 h-4 rounded-full border border-gray-300" style={{ backgroundColor: state.colors.background.secondary }}></div>
                        ...
                        {/* Button Colors */}
                        <div className="space-y-2">
                            <p className="text-xs font-medium text-gray-600 uppercase">Botones</p>
                            <div className="flex items-center gap-2">
                                <div className="w-4 h-4 rounded-full border border-gray-300" style={{ backgroundColor: state.colors.buttons.primary.bg }}></div>
                                <span className="text-xs text-gray-600">Primario</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <div className="w-4 h-4 rounded-full border border-gray-300" style={{ backgroundColor: state.colors.buttons.secondary.bg }}></div>
                                <span className="text-xs text-gray-600">Secundario</span>
                            </div>
                        </div>
                    </div>
