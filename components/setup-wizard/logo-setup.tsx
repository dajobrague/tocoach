        ...
        <div className="space-y-6">
            <div>
                <h4 className="text-md font-semibold text-black mb-4 flex items-center gap-2">
                    <Icon icon="solar:gallery-linear" className="text-blue-600" />
                    Logo y Marca
                </h4>
            </div>
            ...
            <div className="space-y-4">
                ...
                {state.logo?.url ? (
                        ...
                        <CardBody className="p-6">
                            <div className="flex items-center gap-4">
                                ...
                                <div className="flex-1">
                                    ...
                                    <p className="text-xs text-gray-500">Haz clic en "Cambiar" para subir uno nuevo</p>
                                </div>
                    ...
                    <div>
                        <div
                            {...getRootProps()}
                            className={`border-2 border-dashed rounded-lg transition-colors cursor-pointer p-8 text-center ${isDragActive
            ...
            <div className="space-y-3">
                <label className="text-sm font-medium text-black">Texto de marca (alternativo)</label>
                <Input
            ...
            {/* Logo Position */}
            <div className="space-y-3">
                <label className="text-sm font-medium text-black">Posición del logo</label>
                <div className="grid grid-cols-3 gap-3">
                    {logoPositionOptions.map((option) => (
                        <Button
                            key={option.key}
                            variant={state.logo?.position === option.key ? 'solid' : 'bordered'}
                            color={state.logo?.position === option.key ? 'primary' : 'default'}
                            className="h-auto p-4 flex-col gap-2"
                            onPress={() => actions.setLogoPosition(option.key as 'left' | 'center' | 'right')}
                        >
            ...
            {/* Logo Size */}
            <div className="space-y-3">
                <label className="text-sm font-medium text-black">Tamaño del logo</label>
                <Select
