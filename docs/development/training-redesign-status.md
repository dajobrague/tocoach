...
- **`main` LOCAL** contiene todo lo mergeado (nutrición + training quick wins + duplicación + rediseño rebanadas 1-4 completas; último merge `0069eac`). **NO PUSHEADO**: `origin/main` = `ac4ad75` (PR #14) = lo deployado. Un `git push` de main deployaría todo → siempre por rama + PR cuando David lo pida.
- Migraciones aplicadas a LOCAL (psql :54422) **y PROD** (MCP `apply_migration`, proyecto `ydqhndnvrkvycnkaghro`): `client_nutrition_visibility` (20260728130000), `exercise_video_reviews` (20260728200000, incluye los valores de enum `video_feedback` y `video_upload` en `notification_type`) y `exercise_logs_progression_index` (20260729090000, solo CREATE INDEX). Ledger caveat de siempre (repair antes de cualquier `db push`).
- Verificación estándar por ronda: `npm run type-check` + `npm run lint` (0 errors; ~1130 warnings pre-existentes OK) + `npx vitest run` (790/790 al cierre de la rebanada Progreso).
- Auditoría de datos rebanada 4 (pedido de David): CERO mutaciones nuevas en el diff, cero mutaciones cambiadas, DDL solo índice — todo el slice es read-and-compute.
...
7. ✅ **1RM + récords + celebración (rebanada 4) — COMPLETO**. `lib/training/e1rm.ts` núcleo puro (Epley ≥7 / Brzycki 2–6 / 1 rep = peso; 17 tests) + `progression-query.ts` (solo logs finalizados; 11 tests) + endpoints gemelos `/progression` (?days 1..730, default 365). Trainer: pill **Progreso** (selector desde el cache de logs — Autocomplete con inputValue controlado, franja de stats con tendencia 3M, chart e1RM|Volumen con rangos 3M/1A/2A, tabla de récords por reps con 🏅 y chip "PR reciente"); el popover del icono de métricas hereda el toggle vía `exercise-volume-chart` compartido + strip de récords; la medalla del detalle del día ahora premia mejor e1RM (no peso suelto, comparación con epsilon). Cliente: "Tu progreso" (sparkline SVG inline, sin recharts a propósito) + chips "Tus récords" en el modal del ejercicio (oculto con <2 puntos); celebración en el PRIMER finalize: `canvas-confetti` (import dinámico, z 100060, respeta reduced-motion) + toast + banner amber en el footer que retiene el modal ~2s; primera marca = línea discreta sin confeti; re-finalize y autosaves nunca celebran. Fix: el PR del historial ya no lo puede ganar un autosave sin finalizar. Detección de récords excluye el log actual por id (inmune al borra-y-reinserta).

   _Pendiente de José_: puede mandar un Loom sobre representación de progresión — si contradice el split Epley/Brzycki, ajustar `lib/training/e1rm.ts` (un solo lugar). Decidido: Epley (reps≥7) + Brzycki (2-6), 1 rep = peso. Diseño validado por Plan agent: TODO computed-on-the-fly (nada almacenado; `personal_records` sigue dormida), `lib/training/e1rm.ts` puro compartido, endpoints gemelos `/progression` (cliente+trainer, filtro finalized_at, `?days` 365/730), detección de récord EN el POST de logs al finalizar (prior bests con `exercise_log_id != current` → response `newRecords|null`, `firstTime` sin confetti), fix del history endpoint (hoy el PR puede ganarlo un autosave), migración índice `exercise_logs (client_id, exercise_id, completed_at DESC)`. UI: pestaña **Progreso** (4ª pill) con chart e1RM (volumen como toggle secundario — decisión David), lista récords por reps ("6×102,5"), celebración cliente = upgrade de PrBanner + addToast (no hay confetti lib). En el detalle del día del trainer YA hay medalla 🏅 por peso máximo histórico (upgrade a e1RM al llegar esta fase).
8. ✅ **Ejercicio de otro día sobre la marcha (cliente) — COMPLETO** (merge `8d37f23`). Botón punteado "Agregar ejercicio de otro día" en la sesión activa → `borrow-exercise-modal.tsx` (todo del cache de programas, cero fetches); el prestado entra a la lista con chip "De: {sesión}" y se loguea normal. CRÍTICO implementado: se descarta el `session_exercise_id` de la sesión de origen (evita el bug de atribución "false done"). Procedencia = `exercise_logs.metadata.borrowed_from_session_name` (el update la PRESERVA aunque un save posterior no la mande — metadata se reconstruye entero en cada save); el detalle del día del trainer etiqueta el fuera-de-plan con chip ámbar "De la sesión «X»". No afecta autocompleción (cobertura = slots planificados). Limitación v1: sin overrides del trainer ni last_used_weights para el prestado.
9. ✅ **Rebanada 5 — limpieza + review COMPLETO** (merge `a68e743`).
   - Review completo (CodeRabbit, base `ac4ad75` = todo lo no deployado): 37 hallazgos, 16 arreglados en `9ee7cb4` — destacan: la API de semana del cliente FILTRABA mal `goals` (viajaba aunque la sección estuviera oculta), PATCH de estado sin scope de client_id, sentinel "all" del multi-select de visibilidad colapsaba a vacío, el drawer perdía intensidades de texto libre al editar, undo-completar degradaba la sesión si fallaba el chequeo de cobertura, guardados del microciclo serializados. Skips documentados en el commit (RLS permisiva = modelo de la casa; trío RPC atómico ya declinado; fire-and-forget ok en proceso Railway de vida larga).
   - Borrados 7 archivos muertos (7.831 líneas): workouts-tab, cardio-tab, calendar-tab, gallery-tab, orphan-exercises-section, exercise-progress-card, use-exercise-expanded-state. Cada uno verificado con búsqueda de referencias repo-wide + tsc + 790/790 tests + BUILD de producción completo en verde. `microcycle-config`/slider/aside ya no existían; `apply-template-section` se CONSERVA (lo usa forms-tab).
   - Pendiente de decisión de producto (no es código muerto): retirar "Categoría" a nivel programa de los modales; posible "Ver pausados" en el selector.
...
- ✅ **Rebanada 4 — Progreso + 1RM** (punto 7).
