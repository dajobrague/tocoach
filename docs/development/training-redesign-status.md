...
- **`main` LOCAL** contiene todo lo mergeado (nutrición + training quick wins + duplicación + rediseño slice 1). **NO PUSHEADO**: `origin/main` = `ac4ad75` (PR #14) = lo deployado. Un `git push` de main deployaría todo → siempre por rama + PR cuando David lo pida.
- Rama actual: `feat/training-programa` (slice Programa completo, ~10 commits), montada sobre local main.
- Migraciones aplicadas a LOCAL (psql :54422) **y PROD** (MCP `apply_migration`, proyecto `ydqhndnvrkvycnkaghro`): `client_nutrition_visibility` (20260728130000). Ledger caveat de siempre (repair antes de cualquier `db push`).
- Verificación estándar por ronda: `npm run type-check` + `npm run lint` (0 errors; ~1120 warnings pre-existentes OK) + `npx vitest run` (758/758 al cierre de esta sesión). Los 3 fallos de meal-logs solo existían en la rama nutrición, no en main.
...
6. ⏳ **PRÓXIMO — Videos tab (rebanada 2 del rediseño)**: sub-superficie "Videos" en el shell del tab Entrenamiento. Feed de todos los videos subidos por el cliente (nuevo→viejo): nombre ejercicio + serie (reps × kg) + fecha + play (reusar `TrainerExerciseVideoModal`) + toggle **Revisado** (revisado = atenuado). Diseño ya definido en los mockups. Backend: migración `exercise_video_reviews` (tenant_host FK, client_id FK, trainer_id, video_url UNIQUE(client_id, video_url), exercise_log_id sin FK, reviewed_at; fila = revisado, quitar = delete; patrón client_diet_pdfs) + PUT `/api/clients/[clientId]/video-reviews` + extender GET exercise-logs/trainer con mapa `videoReviews` por URL. La URL es la clave estable (sets se borran/reinsertan en cada autosave). Aplicar migración local+prod al cerrar.
7. ⏳ **1RM + récords + celebración (Fase 4 / rebanada 4)** — EL ITEM ESTRELLA para José. Decidido: Epley (reps≥7) + Brzycki (2-6), 1 rep = peso. Diseño validado por Plan agent: TODO computed-on-the-fly (nada almacenado; `personal_records` sigue dormida), `lib/training/e1rm.ts` puro compartido, endpoints gemelos `/progression` (cliente+trainer, filtro finalized_at, `?days` 365/730), detección de récord EN el POST de logs al finalizar (prior bests con `exercise_log_id != current` → response `newRecords|null`, `firstTime` sin confetti), fix del history endpoint (hoy el PR puede ganarlo un autosave), migración índice `exercise_logs (client_id, exercise_id, completed_at DESC)`. UI: pestaña **Progreso** (4ª pill) con chart e1RM (volumen como toggle secundario — decisión David), lista récords por reps ("6×102,5"), celebración cliente = upgrade de PrBanner + addToast (no hay confetti lib). En el detalle del día del trainer YA hay medalla 🏅 por peso máximo histórico (upgrade a e1RM al llegar esta fase).
...
- ⏳ **Rebanada 2 — Videos** (punto 6 arriba).
