      ...
      <div className="p-5">
        {state.isError ? (
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-7">
              ...
              return (
                <div key={day} className="group relative">
                  ...
                  >
                    <PopoverTrigger>
                      <button
                        className={`flex min-h-[4.5rem] w-full flex-col items-start gap-1 rounded-large p-2.5 text-left transition-colors ${
                          resolved !== null
                  ...
                  {/* × al hover para quitar el día (hermano del trigger, como
                      el day-selector de nutrición). Con sesión asignada pide
                      confirmación; vacío se quita directo. */}
                  {state.durationDays > 1 ? (
                    <button
                      aria-label={`Quitar día ${day}`}
                      className="absolute right-1 top-1 flex h-4.5 w-4.5 items-center justify-center rounded-full bg-white text-gray-600 opacity-0 shadow-sm ring-1 ring-gray-200 transition-opacity hover:text-danger group-hover:opacity-100"
                      type="button"
                      onClick={(event) => {
            ...
            {/* Tile punteado para agregar un día al final (como "+ Día" en
                el plan de nutrición). */}
            {state.durationDays < MAX_DAYS ? (
              <button
                className="flex min-h-[4.5rem] flex-col items-center justify-center gap-1 rounded-large border border-dashed border-gray-300 text-xs font-medium text-default-500 transition-all hover:border-blue-400 hover:bg-blue-50/40 hover:text-blue-600"
                type="button"
