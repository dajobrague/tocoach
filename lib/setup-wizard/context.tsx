...
type SetupWizardAction =
    ...
    | { type: 'SET_DOMAIN_AVAILABILITY'; payload: { isAvailable: boolean; suggestions: string[] } }
    | { type: 'SET_DOMAIN_CHECKING'; payload: boolean }
    | { type: 'SET_PRIMARY_COLOR'; payload: string }
    | { type: 'SET_SECONDARY_COLOR'; payload: string }
    | { type: 'SET_TEXT_COLOR'; payload: { type: 'h1' | 'h2' | 'h3' | 'body' | 'muted'; color: string } }
    | { type: 'SET_BACKGROUND_COLOR'; payload: { type: 'primary' | 'secondary' | 'accent'; color: string } }
    | { type: 'SET_BUTTON_COLOR'; payload: { button: 'primary' | 'secondary'; property: 'bg' | 'text' | 'hover'; color: string } }
    | { type: 'SET_SHADOW_COLOR'; payload: { type: 'light' | 'medium' | 'dark'; color: string } }
...
// Reducer
function setupWizardReducer(state: SetupWizardState, action: SetupWizardAction): SetupWizardState {
    switch (action.type) {
        ...
        case 'SET_TEXT_COLOR':
            return {
                ...
                colors: {
                    ...state.colors,
                    text: { ...state.colors.text, [action.payload.type]: action.payload.color },
                },
            };
        ...
        case 'SET_BACKGROUND_COLOR':
            return {
                ...
                colors: {
                    ...state.colors,
                    background: { ...state.colors.background, [action.payload.type]: action.payload.color },
                },
            };
        ...
        case 'SET_SHADOW_COLOR':
            return {
                ...
                colors: {
                    ...state.colors,
                    shadows: { ...state.colors.shadows, [action.payload.type]: action.payload.color },
                },
            };
...
const SetupWizardContext = createContext<{
    ...
    actions: {
        ...
        setTextColor: (type: 'h1' | 'h2' | 'h3' | 'body' | 'muted', color: string) => void;
        setBackgroundColor: (type: 'primary' | 'secondary' | 'accent', color: string) => void;
        setButtonColor: (button: 'primary' | 'secondary', property: 'bg' | 'text' | 'hover', color: string) => void;
...
export function SetupWizardProvider({
    ...
    const checkDomainAvailability = useCallback(async (domain: string) => {
        ...
        try {
            ...
            dispatch({
                type: 'SET_ERROR',
                payload: { field: 'domain', message: 'Error al verificar disponibilidad' },
            });
        }
    }, []);
    ...
    const saveDomain = useCallback(async (domain: string) => {
        ...
        try {
            ...
            dispatch({
                ...
                payload: {
                    field: 'domain',
                    message: error instanceof Error ? error.message : 'Error al guardar el dominio'
                },
            });
    ...
    const setTextColor = useCallback((type: 'h1' | 'h2' | 'h3' | 'body' | 'muted', color: string) => {
        dispatch({ type: 'SET_TEXT_COLOR', payload: { type, color } });
    }, []);

    const setBackgroundColor = useCallback((type: 'primary' | 'secondary' | 'accent', color: string) => {
        dispatch({ type: 'SET_BACKGROUND_COLOR', payload: { type, color } });
    }, []);

    const setButtonColor = useCallback((button: 'primary' | 'secondary', property: 'bg' | 'text' | 'hover', color: string) => {
        dispatch({ type: 'SET_BUTTON_COLOR', payload: { button, property, color } });
    }, []);

    const setShadowColor = useCallback((type: 'light' | 'medium' | 'dark', color: string) => {
    ...
    const setLogoPosition = useCallback((position: 'left' | 'center' | 'right') => {
