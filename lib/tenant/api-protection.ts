...
export async function withTenantProtection(
    ...
    try {
        ...
        console.log(`[API Protection] Checking tenant for host: ${normalizedHost}`, { correlationId });

        // Load tenant context
        const tenantContext = await loadTenantContext(normalizedHost);

        if (!tenantContext) {
            console.warn(`[API Protection] Unknown host: ${normalizedHost}`, { correlationId });
        ...
        console.error(`[API Protection] Protection check failed for host: ${request.headers.get("host")}`, {
            correlationId,
