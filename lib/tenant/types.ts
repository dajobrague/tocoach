...
export function assertNoSecrets(obj: any): void {
  ...
  const foundDangerous = dangerousKeys.filter(
    (key) => key in obj || typeof obj[key] === "function"
  );

  if (foundDangerous.length > 0) {
    throw new Error(
      `Security violation: Found dangerous keys in client data: ${foundDangerous.join(", ")}`
    );
  }
